...
function getPaymentStatusTextByMethod(status, method) {
  ...
  if (s === "failed") return "Thất bại";
  if (s === "pending") return "Chờ thanh toán";
  // Fallback cho COD khi chưa có payment record
  if (String(method || "").toLowerCase() === "cod") return "Chờ thanh toán";
...
function renderOrderActions(detail) {
  ...
  // Kiểm tra nếu là thanh toán online đã thành công (PayOS hoặc Momo)
  const isPaidOnline = 
    ["momo", "payos"].includes(paymentMethod) ||
    ["momo", "payos"].includes(provider1) ||
    ["momo", "payos"].includes(provider2) ||
    ["momo", "payos"].includes(provider3);
  ...
  if (canCancel) {
    actionsEl
      ...
      ?.addEventListener("click", async () => {
        ...
        // Nếu là đơn đã trả tiền online: chuyển hướng sang trang cancel.html để nhập thông tin hoàn tiền
        if (isPaidOnline) {
          const nextUrl = `/cancel.html?order_code=${encodeURIComponent(code)}`;
